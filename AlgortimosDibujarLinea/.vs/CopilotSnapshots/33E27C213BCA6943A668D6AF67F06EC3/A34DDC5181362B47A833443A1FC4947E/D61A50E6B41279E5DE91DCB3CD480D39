...
namespace AlgortimosDibujarLinea
{
    public partial class frmMetodoRellenoBoundary : Form
    {
        ...
        private async void BtnCalcular_Click(object sender, EventArgs e)
        {
            ...
            // dilate boundary by 1 pixel in all directions
            ...
            var fillPoints = await Task.Run(() => CRelleno.BoundaryFillSeed(seed, boundarySet, false));
            ...
            // Animate fill pixel-by-pixel. Use full cell size (scale) to avoid gaps.
            int widthPx = pcbGrafico.Width;
            int heightPx = pcbGrafico.Height;
            using (Graphics g = Graphics.FromImage(pcbGrafico.Image))
            using (var fillBrush = new SolidBrush(Color.FromArgb(200, 220, 180, 80)))
            using (var penFill = new Pen(Color.FromArgb(160, 120, 80, 30), 1))
            {
        ...
        {
            // Event handler left intentionally empty to satisfy designer hookup.
        }
    }
}
