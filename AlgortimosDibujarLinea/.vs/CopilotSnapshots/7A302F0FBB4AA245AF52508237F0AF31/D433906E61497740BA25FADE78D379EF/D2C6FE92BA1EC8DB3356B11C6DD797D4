...
{
    /// <summary>
    /// Formulario para recorte de polígonos con la variante Weiler-Atherton (implementación por defecto / fallback).
    /// Permite definir un polígono sujeto mediante clics y visualizar el resultado del recorte frente a
    /// la ventana de recorte (área del PictureBox).
    /// </summary>
    public partial class frmMetodoRecortePoligonoWeiler : Form
    {
        private List<PointF> poligonoSujeto = new List<PointF>();
        private List<PointF> poligonoRecorte = null;

        public frmMetodoRecortePoligonoWeiler()
        {
            InitializeComponent();
        ...
        private void Redraw()
        {
            ...
            using (Graphics g = Graphics.FromImage(pcbGrafico.Image))
            {
                ...
                // Dibujar rectángulo de recorte (ventana)
                ...
                // Si el polígono sujeto es válido (>=3 vértices) -> realizar recorte
                if (poligonoSujeto.Count>=3)
                {
                    var resultado = CRecortePoligono.WeilerAthertonClip(poligonoSujeto, poligonoRecorte);
                    if (resultado!=null && resultado.Count>0)
                    {
                        var rpts = resultado.Select(p=> new PointF(cx + p.X*scale, cy - p.Y*scale)).ToArray();
                        using (var brush = new SolidBrush(Color.FromArgb(120, 200, 100, 100))) g.FillPolygon(brush, rpts);
                        g.DrawPolygon(new Pen(Color.Red,2), rpts);
                    }
                }
            }
