...
namespace AlgortimosDibujarLinea
{
    ...
    public partial class frmMetodoCorteLineaCohen : Form
    {
        ...
        private void PcbGrafico_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            if (puntoA == null)
            {
                puntoA = new Point(lx, ly);
            }
            else if (puntoB == null)
            {
                puntoB = new Point(lx, ly);
            }
            else
            {
                puntoA = new Point(lx, ly);
        ...
        private void DrawPointsAndLines()
        {
            ...
            using (Graphics g = Graphics.FromImage(pcbGrafico.Image))
            {
                if (puntoA != null)
                {
                    var p = map(puntoA.Value.X, puntoA.Value.Y);
                    g.FillEllipse(Brushes.Red, p.X - 3, p.Y - 3, 6, 6);
                }
                if (puntoB != null)
                {
                    var p = map(puntoB.Value.X, puntoB.Value.Y);
                    g.FillEllipse(Brushes.Red, p.X - 3, p.Y - 3, 6, 6);
                }

                if (puntoA != null && puntoB != null)
                {
                    var pA = map(puntoA.Value.X, puntoA.Value.Y);
                    var pB = map(puntoB.Value.X, puntoB.Value.Y);
                    ...
                    bool aceptado = CRecorteLinea.CohenSutherlandClip(ref cp0, ref cp1, new Rectangle(xmin, ymin, xmax - xmin, ymax - ymin));
